        ...
        if self.task == "connected_cube":
            ...
            if self.extra_params["generate_sft_data"]:
                ...
                if self.traintest == "train":
                    task_idx = random.choice(list(self.generated_connected_cube_set.keys()))
                    cube_xyz_idx = self.generated_connected_cube_set[task_idx]["task"]
                    self.generated_connected_cube_set_solution = self.generated_connected_cube_set[task_idx]["solution"]
            ...
            cube_xyz_idx = self.generate_mrt(
                15 # actual number of blocks is this param - 3
            )
        else:
            raise NotImplementedError(f"Unknown task: {self.task}")
...
def create_env(task="connected_cube", extra_params={}):
    ...
    env_config = {
        ...
        "use_camera_obs": True,
        "is_gravity": extra_params.get("is_gravity", True),
    }
    # Load controller
